...
corpus <- read.csv("corpus.csv")
 # gutenberg_download(mirror="http://mirrors.xmission.com/gutenberg/", meta_fields = #c("author","title"))
```
...
write.csv(corpus_text, "corpus_text.csv")
...
```{r}
clean_entities <- read.csv("entities_raw.csv")
```
...
In the previous version, the `words` column was renamed `entity` due to the way the fuzzy join renamed the columns, as a result there is now some old cold that still abides by that naming convention. You can fix the issue by running the lines below.
